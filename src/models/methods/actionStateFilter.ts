...
const resolveStates = (
  ...
  return arr.flatMap((id: string | number) => {
    if (id === 'custom' || id == 'custom') return customIds || []
    const s = actionStates.find((a: any) => a.id == id)
    if (!s) return []
...
export default (
  ...
  params: ActionStateFilterParams,
  ...
  const limit =
    maxFiles === '' || maxFiles == null
      ? prevResults.length
      : Math.max(0, parseInt(maxFiles, 10) || prevResults.length)
  return prevResults.slice(0, limit).filter((result, index) => {
    eventEmitter({ current: index, total: limit })
    ...
    const matchP1 = (post: any) => {
      if (hasP1 && p1States.indexOf(post.actionStateId) == -1) return false
      if (
        hasP1Pos &&
        !checkPos(post, _comboerMinX, _comboerMaxX, _comboerMinY, _comboerMaxY)
      )
        return false
      return true
    }
    const matchP2 = (post: any) => {
      if (hasP2 && p2States.indexOf(post.actionStateId) == -1) return false
      if (
        hasP2Pos &&
        !checkPos(post, _comboeeMinX, _comboeeMaxX, _comboeeMinY, _comboeeMaxY)
      )
        return false
      return true
    }
