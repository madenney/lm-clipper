...
export type UseResultsFetcherParams = {
  ...
  showingCountStore: { set: (_value: number) => void }
}
...
export const useResultsFetcher = (params: UseResultsFetcherParams) => {
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      debugRef.current.setTaskLabel(
        `fetching ${windowRange.limit.toLocaleString()}`,
      )
      debugRef.current.logZoomTrace('fetch_start', {
        ...
        lite:
          webglWanted ||
          useWebglMode ||
          visibleRange.limit >= liteResultsThreshold,
      })
      ...
      ipcBridge.getResults(
        ...
        (response) => {
          ...
          if (useWebglMode) {
            ...
            webglDataRangeRef.current = {
              offset: windowRange.offset,
              limit: items.length,
            }
            ...
            setCurrentResults([])
            debugRef.current.setTaskLabel(
              `rendering ${items.length.toLocaleString()}`,
            )
            debugRef.current.logZoomTrace('render_webgl_data', {
              count: items.length,
            })
          ...
          debugRef.current.setTaskLabel(
            `rendering ${newResults.length.toLocaleString()}`,
          )
