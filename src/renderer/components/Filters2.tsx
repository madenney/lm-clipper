...
export default function Filters({
  ...
  function runFilter(filter: ShallowFilterInterface) {
    ...
    ipcBridge.runFilter(filter.id, (response) => {
      ...
      // Merge: only update filters that aren't currently running
      setArchive((prev) => {
        if (!prev || !response.filters) return response
        const merged = {
          ...response,
          filters: response.filters.map((rf: ShallowFilterInterface) => {
            const prevFilter = prev.filters.find((pf) => pf.id === rf.id)
            if (!prevFilter) return rf
            // If this filter is currently running in the UI, keep its previous display state
            const rfIndex = prev.filters.indexOf(prevFilter)
            if (runningFilters.has(rfIndex) && rf.id !== filter.id) {
              return prevFilter
            }
            return rf
          }),
