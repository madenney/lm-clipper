...
export default function Results({
  ...
  function renderList() {
    ...
    return results.map((result, index) => {
      const { stage, comboer, comboee, players } = result
      if (!comboer && !comboee && !players) throw Error('?')
      const stageTag = stages[stage as keyof typeof stages].tag
