...
const getInfoFromSlpFile = function getInfoFromSlpFile(file) {
  ...
  if (Number.isNaN(length)) {
    return { invalid: true, reason: 'No metadata' }
  }
  if (length < 45) {
    return { invalid: true, reason: 'Game Length < 45 seconds' }
  }
  ...
  if (!gameComplete) {
    return { invalid: true, reason: 'Game incomplete' }
  }
  if (overall.every((p) => p.totalDamage < 100)) {
    return { invalid: true, reason: 'Damage < 100' }
  }
  if (overall.some((p) => p.inputCount < 100)) {
    return { invalid: true, reason: 'InputCount < 100' }
  }
  const p1Stocks = stocks.filter(
    (stock) => stock.playerIndex === p1.playerIndex,
  )
  const p2Stocks = stocks.filter(
    (stock) => stock.playerIndex === p2.playerIndex,
  )
  // TODO: Come back to this
  if (p1Stocks.length < 4 && p2Stocks.length < 4) {
    return { invalid: true, reason: 'Neither player lost 4 stocks' }
  }

  // Get Winner
  let winnerIndex
  if (p1Stocks.length < 4) {
    winnerIndex = p1Stocks[0].playerIndex
  }
  if (p2Stocks.length < 4) {
  ...
  if (typeof winnerIndex === 'undefined') {
    throw new Error("Somehow didn't determine a winner.")
  }

  let _winner
  let _loser
  if (winnerIndex === settings.players[0].playerIndex) {
    _winner = settings.players[0]
    _loser = settings.players[1]
  } else {
    _winner = settings.players[1]
    _loser = settings.players[0]
  }
...
function getDeepInfoFromSlpFile(file) {
  ...
  const settings = game.getSettings()
  const metadata = game.getMetadata()
  const _length = metadata.lastFrame / 60
